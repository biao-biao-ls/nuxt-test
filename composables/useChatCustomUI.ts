...
export class ChatCustomUI {
    ...
    try {
      if (typeof localStorage !== "undefined") {
        ...
        if (agent) {
          ...
          console.log(`已保存选择的客服到本地存储: ${agent.employeeEnName}`);
        } else {
  ...
  restorePreviousSelectedAgent(): void {
    ...
    if (!storedAgent) {
      console.log("没有找到之前选择的客服记录");
      return;
    }

    console.log(
      `尝试恢复之前选择的客服: ${storedAgent.employeeEnName} (ID: ${storedAgent.quickCepId})`
    );
    ...
    if (!agent) {
      console.log(`客服 ${storedAgent.employeeEnName} 不存在于当前客服列表中`);
      this.saveSelectedAgent(null); // 清除无效的存储
      return;
    }

    if (!agent.isOnline) {
      console.log(`客服 ${storedAgent.employeeEnName} 当前离线，无法自动恢复`);
      return;
    }

    // 如果客服在线，自动切换到该客服
    console.log(`客服 ${storedAgent.employeeEnName} 在线，自动恢复选择`);
  ...
  updateAgentStatus(operatorUserIdStatus: Record<string, number>): void {
    console.log("收到客服状态更新数据:", operatorUserIdStatus);
    ...
    this.state.customerServiceData.forEach((agent, index) => {
      if (operatorUserIdStatus.hasOwnProperty(agent.quickCepId)) {
        ...
        agent.isOnline = index < 1 ? true : false;
        ...
        if (
          ...
          console.log(
            `当前客服 ${agent.employeeEnName} 已离线，将恢复为默认客服`
          );
        }
        // } else {
        //   console.log(`客服 ${agent.employeeEnName} 状态无变化，跳过更新`);
        // }
      }
    });
    ...
    if (hasStatusChanged) {
      ...
      this.emitCustomEvent("agentStatusUpdated", {
        ...
        currentAgentWentOffline: currentAgentWentOffline,
      });
    } else {
      console.log("所有客服状态均无变化，跳过UI更新");
    }
  }
  ...
  checkCurrentAgentStatus(): boolean {
    ...
    if (!currentAgent || !currentAgent.isOnline) {
      console.log(
        `当前客服 ${this.state.currentChatAgent.employeeEnName} 已离线，自动恢复为默认客服`
      );
      this.state.currentChatAgent = null;
   ...
   */
  selectAgent(quickCepId: string, saveToStorage: boolean = true): void {
    const agent = this.state.customerServiceData.find(
  ...
  generateHeaderHTML(): string {
    ...
    // 根据显示规则决定显示的客服数量
    const displayAgents = shouldShowHeaderAgents
      ? availableAgents.slice(0, 3)
      : [];

    // 决定是否显示更多客服指示器
    const shouldShowMoreIndicator =
      shouldShowHeaderAgents && availableCount > 3;
        ...
        <div class="chat-header-agents">
          ...
          ${
            shouldShowMoreIndicator
              ? this.renderMoreAgentsIndicator(availableCount)
              : ""
          }
  ...
  private renderCurrentAgent(): string {
    ...
    return `
      <div class="current-agent selected" title="当前沟通: ${
  ...
  showTooltip(
    ...
    if (tooltip) {
      ...
      console.log(
        `Tooltip显示: ${name}, 位置: ${event.clientX + 12}, ${
          event.clientY - 50
        }`
      );
    } else {
  ...
  showFullTooltip(
    ...
    if (tooltip) {
      ...
      console.log(
        `Full Tooltip显示: ${name}, 位置: ${event.clientX + 15}, ${
          event.clientY - 60
        }`
      );
    }
  ...
  private positionTooltipInIframe(
    ...
    if (isInIframe) {
      ...
      console.log(`Tooltip positioned in iframe: left=${left}, top=${top}`);
    } else {
      // 不在 iframe 中，使用原有逻辑
      const viewportWidth = doc.documentElement.clientWidth;
      const viewportHeight = doc.documentElement.clientHeight;
  ...
  private createTooltipElement(
    ...
    if (targetContainer) {
      ...
      console.log(`已创建tooltip元素: ${id}，添加到:`, targetContainer.tagName);
      return tooltip;
    }
