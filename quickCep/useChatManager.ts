  ...
  private async handleOperatorListChange(data: any): Promise<void> {
    ...
    if (data && Array.isArray(data) && data.length > 0) {
      ...
      for (const item of data) {
        ...
        const matchedAgent = customerServiceData.find((agent) => agent.quickCepId === operatorId)
        ...
        if (matchedAgent) {
          ...
          // 通过接口查询客服信息
          try {
            const apiAgent = await getImEmployeeInfoByQuickCepId(operatorId)
            if (apiAgent) {
              newData.push({
                ...apiAgent,
                isOnline: true,
              ...
              console.log('通过API成功获取客服信息:', apiAgent)
            } else {
              console.warn('API也未找到匹配的客服，operatorId:', operatorId)
            }
          } catch (error) {
            console.error('查询客服信息失败，operatorId:', operatorId, error)
          }
        }
      }
    ...
    window.debugQuickChat = {
      ...
      checkCloseBoxStatus: () => {
        try {
          ...
          if (closeBox) {
            const isHidden = closeBox.style.display === 'none' || window.getComputedStyle(closeBox).display === 'none'
            console.log('✅ 找到关闭按钮:', {
              exists: true,
              isHidden,
      ...
      inspectQuickChatApi: () => {
        if (typeof window !== 'undefined' && window.quickChatApi) {
          ...
          console.log(
            '💬 基础聊天功能:',
            chatMethods.filter((method) => typeof window.quickChatApi![method] === 'function')
          )
          ...
          console.log(
            '📨 消息相关方法:',
            messageMethods.filter((method) => typeof window.quickChatApi![method] === 'function')
          )
          ...
          console.log(
            '👤 用户管理方法:',
            userMethods.filter((method) => typeof window.quickChatApi![method] === 'function')
          )
          ...
          console.log(
            '🎧 座席管理方法:',
            operatorMethods.filter((method) => typeof window.quickChatApi![method] === 'function')
          )
          ...
          console.log(
            '🎨 界面控制方法:',
            uiMethods.filter((method) => typeof window.quickChatApi![method] === 'function')
          )
          ...
          console.log(
            '⚡ 事件系统方法:',
            eventMethods.filter((method) => typeof window.quickChatApi![method] === 'function')
          )
    ...
    if (typeof window !== 'undefined' && window.quickChatApi) {
      ...
      try {
        ...
        if (window.quickChatApi.sendMessage) {
          await window.quickChatApi.sendMessage(orderMessage)
